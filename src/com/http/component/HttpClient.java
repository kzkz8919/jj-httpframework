...
public class HttpClient {
	...
	public String get(String url) {
		...
		try {
			...
			RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(Config.timeout)
					.setConnectTimeout(Config.timeout).build();// 构建RequestConfig对象
			httpGet = new HttpGet(url);// 构建HttpGet对象
			httpGet.setConfig(requestConfig);// 设置RequestConfig
			CloseableHttpResponse response = httpClient.execute(httpGet);// ִ执行请求并获取response对象
	...
	public String post(String url, Map<String, String> map) {
		...
		try {
			...
			/**
			 * new UrlEncodedFormEntity(ps)中的UrlEncodedFormEntity对象是将List
			 * <NameValuePair>转换为表单参数的数据格式，
	...
	public String post(String url, String body) {
		...
		try {
			httpClient = HttpClients.createDefault();// 实例化HttpClient对象
			RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(2000).setConnectTimeout(2000).build();// 构建RequestConfig对象
			httpPost = new HttpPost(url);/// 构建HttpPost对象
			httpPost.setConfig(requestConfig);// 设置RequestConfig
			/**
			 * 在请求正文中，可以随意的提交一段字符串给服务器，现在为了数据交换的统一性，所以大家都是提交的一段json串，
			 * 所以body的数据格式{"name":"zhangsan","age":18}，如果body是该数据格式，则Content-
			 * Type要设置为application/json new
			 * StringEntity(body)是指利用body构建一个请求正文对象，且StringEntity是支持Content-
			 * Type为application/json的，
